...
//         // Set empty contacts if fetch fails
//         setEmergencyContacts([]);
//       }
//     } else {
...
useEffect(() => {
  ...
  const timeoutId = setTimeout(() => {
    ...
    if (mounted) {
      setLoadingStep("Connection timeout...");
      setLoading(false);
      toast({
  ...
  const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
    ...
    if (currentUser) {
      ...
      try {
        ...
        // ✅ Properly handle document existence
        if (docSnap.exists()) {
          const data = docSnap.data();
          if (data?.emergencyContacts && Array.isArray(data.emergencyContacts)) {
            console.log("✅ Contacts loaded:", data.emergencyContacts.length);
            setEmergencyContacts(data.emergencyContacts);
          } else {
            console.log("ℹ️ Document exists but no contacts");
            setEmergencyContacts([]);
          }
        } else {
          // ✅ Document doesn't exist - create it for new users
          console.log("ℹ️ Creating new user document");
      ...
      setLoadingStep("Redirecting to login...");
      window.location.href = `${window.location.origin}/auth`;
      return;
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50 p-4 md:p-8">
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom duration-700">
          ...
          <Link to="/disease-predictor" className="group">
            <Card className="h-full bg-white border-0 shadow-xl hover:shadow-2xl hover:shadow-violet-200/60 transition-all duration-500 hover:scale-105 hover:-translate-y-2 transform cursor-pointer overflow-hidden relative">
              <div className="absolute inset-0 bg-gradient-to-br from-violet-100 to-purple-100 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <CardHeader className="relative z-10 pb-4">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-400 to-purple-500 flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300">
                  <span className="text-3xl">⚕️</span>
                </div>
                <CardTitle className="flex items-center space-x-2 text-xl font-bold text-gray-800">
                  <span>Disease Predictor</span>
                </CardTitle>
                <CardDescription className="text-gray-600 font-medium">AI-powered health analysis</CardDescription>
              </CardHeader>
              <CardContent className="relative z-10">
                <p className="text-sm text-gray-500 leading-relaxed">
                  Get instant health insights based on your symptoms with ML predictions
                </p>
              </CardContent>
            </Card>
          </Link>
